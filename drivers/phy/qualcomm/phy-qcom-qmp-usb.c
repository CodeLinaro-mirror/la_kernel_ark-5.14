...
static const struct qmp_phy_cfg sa7255p_usb3_uniphy_cfg = {
	.lanes			= 1,
...
static void qmp_usb_fw_managed_domain_remove(struct qmp_usb *qmp)
{
	dev_pm_domain_detach_list(qmp->pd_list);
}

static int qmp_usb_fw_managed_domain_get(struct qmp_usb *qmp)
{
	...
	if (!dev->pm_domain) {
		...
		qmp->transfer_pd = qmp->pd_list->pd_devs[0];
		qmp->core_pd = qmp->pd_list->pd_devs[1];
	} else
		return -ENODEV;
...
static int qmp_usb_init(struct phy *phy)
{
	...
	if (qmp->fw_managed) {
		ret = pm_runtime_resume_and_get(qmp->core_pd);
		if (ret < 0) {
			dev_err(qmp->dev,
				"Failed to enable fw managed resources");
			return ret;
		}

		ret = pm_runtime_resume_and_get(qmp->transfer_pd);
		if (ret < 0) {
			dev_err(qmp->dev,
				"Failed to enable fw managed resources");
...
err_disable_regulators:
	if (qmp->fw_managed)
		pm_runtime_put_sync(qmp->core_pd);
...
static int qmp_usb_exit(struct phy *phy)
{
	...
	if (qmp->fw_managed) {
		ret = pm_runtime_put_sync(qmp->transfer_pd);
		if (ret < 0)
			dev_err(qmp->dev,
				"Failed to disable fw managed resources");

		ret = pm_runtime_put_sync(qmp->core_pd);
		if (ret < 0)
			dev_err(qmp->dev,
				"Failed to disable fw managed resources");
	}
...
static int qmp_usb_probe(struct platform_device *pdev)
{
	...
	qmp->fw_managed  = device_property_read_bool
				(dev, "qcom,firmware-managed-resources");
	if (qmp->fw_managed) {
		ret = qmp_usb_fw_managed_domain_get(qmp);
		if (ret) {
...
err_node_put:
	of_node_put(np);
	if (qmp->fw_managed)
		qmp_usb_fw_managed_domain_remove(qmp);
	return ret;
}
...
static void qmp_usb_remove(struct platform_device *pdev)
{
	...
	if (qmp->fw_managed) {
		pm_runtime_put_sync(qmp->transfer_pd);
		pm_runtime_put_sync(qmp->core_pd);
		qmp_usb_fw_managed_domain_remove(qmp);
	}
}
...
static const struct of_device_id qmp_usb_of_match_table[] = {
	{
		...
		.compatible = "qcom,sa7255p-qmp-usb3-uni-phy",
		.data = &sa7255p_usb3_uniphy_cfg,
	}, {
